...
interface IZkConnectVerifier {
    ...
    error NamespaceMismatch(
        bytes16 requestNamespace,
        bytes16 responseNamespace
    );
    error AppIdMismatch(bytes16 requestAppId, bytes16 responseAppId);
    error MessageSignatureMismatch(
    ...
    error AuthExtraDataMismatch(
        bytes requestExtraData,
        bytes responseExtraData
    );
    // Claim mismatch errors
    error ClaimTypeMismatch(
        ClaimType requestClaimType,
        ClaimType responseClaimType
    );
    error ClaimValueMismatch(
        uint256 requestClaimValue,
        uint256 responseClaimValue
    );
    error ClaimExtraDataMismatch(
        bytes requestExtraData,
        bytes responseExtraData
    );
    error ClaimGroupIdMismatch(bytes16 requestGroupId, bytes16 responseGroupId);
    error ClaimGroupTimestampMismatch(
        bytes16 requestGroupTimestamp,
        bytes16 responseGroupTimestamp
    );
