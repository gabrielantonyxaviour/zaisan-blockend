...
import {HydraS2Verifier as HydraS2SnarkVerifier} from "@sismo-core/hydra-s2/contracts/HydraS2Verifier.sol";
...
contract HydraS2Verifier is
    IHydraS2Verifier,
    IBaseVerifier,
    HydraS2SnarkVerifier,
    Initializable
{
    ...
    constructor(
        ...
        COMMITMENT_MAPPER_REGISTRY = ICommitmentMapperRegistry(
            commitmentMapperRegistry
        );
        AVAILABLE_ROOTS_REGISTRY = IAvailableRootsRegistry(
        ...
        )
    {
        ...
        VerifiedAuth memory verifiedAuth = _verifyAuthValidity(
            snarkInput,
            auth,
            appId
        );
        VerifiedClaim memory verifiedClaim = _verifyClaimValidity(
            snarkInput,
            claim,
            appId,
            namespace
    ...
    function _verifyClaimValidity(
        ...
        if (input.requestIdentifier != expectedRequestIdentifier) {
            revert RequestIdentifierMismatch(
                input.requestIdentifier,
                expectedRequestIdentifier
            );
        }

        // commitmentMapperPubKey
        uint256[2] memory commitmentMapperPubKey = COMMITMENT_MAPPER_REGISTRY
            .getEdDSAPubKey();
        if (
        ...
        // accountsTreeValue
        uint256 groupSnapshotId = _encodeAccountsTreeValue(
            claim.groupId,
            claim.groupTimestamp
        );
        if (input.accountsTreeValue != groupSnapshotId) {
            revert AccountsTreeValueMismatch(
                input.accountsTreeValue,
                groupSnapshotId
            );
        }
    ...
    function _verifyAuthValidity(
        ...
        if (auth.authType == AuthType.ANON) {
            ...
            // commitmentMapperPubKey
            uint256[2]
                memory commitmentMapperPubKey = COMMITMENT_MAPPER_REGISTRY
                    .getEdDSAPubKey();
            if (
                input.commitmentMapperPubKey[0] != commitmentMapperPubKey[0] ||
                input.commitmentMapperPubKey[1] != commitmentMapperPubKey[1]
    ...
    function _validateSignedMessageInput(
        ...
        if (input.extraData != uint256(keccak256(signedMessage))) {
            revert InvalidExtraData(
                bytes32(input.extraData),
                keccak256(signedMessage)
            );
        }
    }

    function _checkSnarkProof(HydraS2ProofData memory snarkProof)
        internal
        view
    {
        if (
    ...
    function _encodeRequestIdentifier(
        ...
        bytes32 groupSnapshotId = _encodeGroupSnapshotId(
            groupId,
            groupTimestamp
        );
        bytes32 serviceId = _encodeServiceId(appId, namespace);
        return
            uint256(keccak256(abi.encodePacked(serviceId, groupSnapshotId))) %
            HydraS2Lib.SNARK_FIELD;
    }

    function _encodeAccountsTreeValue(bytes16 groupId, bytes16 groupTimestamp)
        internal
        pure
        returns (uint256)
    {
        return
            uint256(_encodeGroupSnapshotId(groupId, groupTimestamp)) %
            HydraS2Lib.SNARK_FIELD;
    }

    function _encodeGroupSnapshotId(bytes16 groupId, bytes16 groupTimestamp)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _encodeServiceId(bytes16 appId, bytes16 namespace)
        internal
        pure
        returns (bytes32)
    {
